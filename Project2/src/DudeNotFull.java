...
public class DudeNotFull extends Dude {
    ...
    public DudeNotFull(Point position, List<PImage> images, int imageIndex, String id, String kind, double animationPeriod, double actionPeriod, int resourceLimit, int resourceCount) {
    ...
    public boolean transform(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.getResourceCount() >= this.getResourceLimit()) {
            Dude dude = new DudeFull(this.getPosition(), this.getImages(), 0, this.getId(),
                    Dude.DUDE_KEY, this.getAnimationPeriod(), this.getActionPeriod(), this.getResourceLimit());
