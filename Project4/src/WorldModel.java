...
public final class WorldModel {
    ...
    public Optional<Entity> findNearest(Point pos, List<String> kinds) {
        ...
        return pos.nearestEntity(ofType, pos, 0);
    }
    ...
    public static void parseEntity(WorldModel world, String line, ImageStore imageStore) {
        ...
        if (properties.length >= Entity.ENTITY_NUM_PROPERTIES) {
            ...
            switch (key) {
                ...
                case Zombie.ZOMBIE_KEY -> world.parseZombie(properties, pt, id, imageStore);
...
//            throw new IllegalArgumentException("position occupied");
            System.out.println("Adding " + entity.getKind() + " failed because the position is already occupied");
        } else {
//            System.out.println("Adding " + entity.getKind() + " succeeded");
    ...
    public void parseSapling(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Sapling.SAPLING_NUM_PROPERTIES) {
            ...
            Entity entity = new Sapling(pt, imageStore.getImageList(Sapling.SAPLING_KEY), 0,  id, Sapling.SAPLING_KEY, Sapling.SAPLING_ACTION_ANIMATION_PERIOD, Sapling.SAPLING_ACTION_ANIMATION_PERIOD, health, Sapling.SAPLING_HEALTH_LIMIT);
    ...
    public void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Dude.DUDE_NUM_PROPERTIES) {
            Entity entity = new DudeNotFull(pt, imageStore.getImageList(Dude.DUDE_KEY), 0, id, Dude.DUDE_KEY,
                    Double.parseDouble(properties[Dude.DUDE_ANIMATION_PERIOD]), Double.parseDouble(properties[Dude.DUDE_ACTION_PERIOD]),
                    Integer.parseInt(properties[Dude.DUDE_LIMIT]), 0, Dude.DUDE_STRATEGY);
            this.tryAddEntity(entity);
        }else{
    ...
    public void parseFairy(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Fairy.FAIRY_NUM_PROPERTIES) {
            Entity entity = new Fairy(pt, imageStore.getImageList(Fairy.FAIRY_KEY),0, id, Fairy.FAIRY_KEY,
                    Double.parseDouble(properties[Fairy.FAIRY_ANIMATION_PERIOD]), Double.parseDouble(properties[Fairy.FAIRY_ACTION_PERIOD]), Fairy.FAIRY_STRATEGY);
            this.tryAddEntity(entity);
        }else{
    ...
    public void parseZombie(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Zombie.ZOMBIE_NUM_PROPERTIES) {
            Entity entity = new Zombie(pt, imageStore.getImageList(Zombie.ZOMBIE_KEY),0, id, Zombie.ZOMBIE_KEY,
                    Double.parseDouble(properties[Zombie.ZOMBIE_ANIMATION_PERIOD]), Double.parseDouble(properties[Zombie.ZOMBIE_ACTION_PERIOD]), Zombie.ZOMBIE_PATHING_STRATEGY);
            this.tryAddEntity(entity);
        }else{
